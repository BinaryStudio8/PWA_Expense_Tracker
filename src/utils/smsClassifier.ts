...
import { BANK_KEYWORDS, Direction, MEMORY_KEY, MEMORY_LIMIT, MERCHANT_KEYWORDS, ScoreMap, TAG_KEYWORDS } from "@/types";
...
function computeHeuristics(text: string) {
    ...
    if (text.includes("paid") || text.includes("debited") || text.includes("withdraw"))
...
// FINAL CLASSIFIER
// -------------------------------
export function classifySms(body: string): ClassificationProps | null {
    if (!body) return null;
...
export function simulateIncoming(body: string) {
    console.log("SIM:", classifySms(body));
}
